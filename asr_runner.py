...
DATASETS_INFO = [
    ...
    ("mozilla-foundation/common_voice_16_1", {"split": "train",       "field": "sentence"}),
    ("facebook/voxpopuli",                   {"split": "test",       "field": "raw_text"}),  # no train split available for accented English in this dataset
    ...
    ("edinburghcstr/ami",                    {"split": "train", "config": "sdm", "field": "text"}),
    # ("distil-whisper/earnings22",            {"split": "test",       "field": "transcription"}),
    # ("MLCommons/peoples_speech",             {"split": "test[:1500]", "field": "text"}),
]

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
MAX_CLIPS_PER_DATASET = 25000        # speed cap; adjust as you like
SAMPLE_RATE = 16_000
...
MODEL_BANK = {
    ...
    "nvidia/parakeet-tdt-0.6b-v2"    : _load_parakeet(),
...
def iter_datasets_and_models():
    ...
    for ds_name, cfg in DATASETS_INFO:
        ...
        if "distil-whisper/earnings22" in ds_name:
            ds_iter = islice(ds_iter, 800)
        else:    
